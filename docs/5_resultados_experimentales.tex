...
En conjunto, estos resultados posicionan a PPO como un algoritmo competitivo en el entorno \textit{SimplePacmanEnv}, especialmente cuando se dispone de un ajuste de hiperparámetros adecuado. Su rendimiento muestra mejoras claras respecto a configuraciones poco optimizadas y resulta particularmente sólido en modos de observación simples, aunque en las configuraciones más informativas DQN alcanza valores superiores de recompensa media y \textit{completion ratio}. Por ello, PPO sigue siendo un candidato relevante para el análisis comparativo presentado en las siguientes secciones, especialmente desde la perspectiva de robustez y sensibilidad al ajuste.
...
Finalmente, la sensibilidad a la complejidad del estado difiere de forma significativa entre algoritmos. En este entorno, DQN muestra una mejora progresiva del rendimiento a medida que se incorporan observaciones más informativas, lo que sugiere que el método basado en valor logra explotar bien la estructura adicional del estado. A2C tolera mejor el cambio de representación en términos de estabilidad, pero no traduce de forma consistente el aumento de información en un mayor rendimiento. PPO, por su parte, puede beneficiarse de estados estructurados, aunque su rendimiento es más dependiente del grado de optimización de hiperparámetros y no domina sistemáticamente a DQN en las configuraciones evaluadas.
...
En conjunto, estos resultados evidencian que no existe un algoritmo universalmente superior, sino que el rendimiento depende tanto del método de aprendizaje como de la representación del estado. En las configuraciones evaluadas, DQN alcanza los mejores valores de \textit{mean reward} y \textit{completion ratio}, especialmente cuando la observación incorpora información más estructurada. PPO se mantiene como una alternativa competitiva y sensible al ajuste de hiperparámetros, mientras que A2C ofrece mayor estabilidad pero un rendimiento claramente inferior. En cualquier caso, ninguno de los métodos logra completar el entorno de forma consistente, manteniendo tasas de éxito nulas.
...
Los resultados experimentales muestran que este modo de observación es uno de los más efectivos, destacando especialmente en DQN, donde se alcanzan los mayores valores de \textit{completion ratio} y recompensa media. Esta representación permite al agente inferir regiones de interés y planificar desplazamientos más coherentes, mejorando la eficiencia del aprendizaje.
