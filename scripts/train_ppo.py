...
# --- Helper to create vecenv workers (Windows/subprocess-friendly) ---
def make_env(obs_mode: str, seed: int, rank: int) -> Callable[[], SimplePacmanEnv]:
    ...
    The seed is offset by `rank` so each subprocess has a distinct RNG stream.
    """
    def _init():
...
def main() -> None:
    ...
    ap.add_argument("--net-arch", type=str, default="128,128")  # "64,64" or "128,128,64", etc.

    # --- Multi-env / rollout size ---
    ...
    ap.add_argument("--clip-range", type=float, default=0.2)  # kept for CLI compatibility
    ap.add_argument("--gae-lambda", type=float, default=0.95)  # kept for CLI compatibility

    args = ap.parse_args()

    # --------- build vecenv ----------
    if args.n_envs > 1:
        # SubprocVecEnv uses processes → more stable & faster for PPO
    ...
    vec = VecNormalize(venv, norm_obs=True, norm_reward=True, clip_reward=10.0)

    is_discrete = isinstance(venv.action_space, Discrete)
    ...
    model = PPO(
        ...
        # You can leave clip_range/gae_lambda as defaults if not passing via CLI
        policy_kwargs=dict(net_arch=net_arch),
        use_sde=use_sde_flag,  # will be False for discrete action spaces
    ...
    vecnorm_path = f"models/vecnorm_{args.obs_mode}_seed{args.seed}_ppo.pkl"
